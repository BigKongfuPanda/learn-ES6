...
foo() // 1 5
/**
 * 上述函数中参数的解构赋值有两层含义： 首先参数为一个对象，其中两个属性 x 和 y 都有默认值，其次，这个对象作
 * 为一个参数也有默认值，其默认值为一个空对象{}。因此，当没有传参而调用函数时，该参数取默认的空对象，然后呢，
 * 在空对象中，去读取 x 和 y 两个属性的时候，发现均为定义，为 undefined，则此时 x 和 y 分别去取各自的默认
 * 值，即 1 和 5。
 */
...
f(); // 1
/**
 * 上面代码中，函数f调用时，参数 y = x 形成一个单独的作用域。这个作用域里面，变量 x 本身没有定义，
...
x // 1
/**
 * 上面代码中，函数foo的参数形成一个单独作用域。这个作用域里面，首先声明了变量x，然后声明了变量y，y的默认值
 * 是一个匿名函数。这个匿名函数内部的变量x，指向同一个作用域的第一个参数x。函数foo内部又声明了一个内部变量
 ...
 * rest 参数搭配的变量是一个数组，注意，rest 参数之后不能再有其他参数（即只能是最后一个参数），否则会报错。
 */
